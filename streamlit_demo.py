...
def main():
    st.set_page_config(page_title="膳魔师(中国)家庭制品有限公司", page_icon=":rainbow:", layout="wide",
    ...
    music = st.sidebar.selectbox('背景音乐:', ['七里香', '稻香'], index=random.choice(range(2)))
    ...
    # t = f'{t}'.split('.')[0]
    # 天气数据刷新时间
    # st.sidebar.write(f'当前时间:{d} {t}')
    chart = st.sidebar.selectbox('当前图表类型', charts_mapping.keys(),
                                 index=st.session_state.random_chart_index)
    city = st.sidebar.selectbox('当前城市', st.session_state.city_mapping.keys(),
                                index=st.session_state.random_city_index)
    # color = st.sidebar.color_picker('颜色选择', '#520520')
    # st.sidebar.write('当前颜色', color)
    ...
    with st.container():
        ...
        col2.metric('温度', forecastToday['temp'])
        col3.metric('体感温度', forecastToday['realFeel'])
        col4.metric('湿度', forecastToday['humidity'])
        col5.metric('风速', forecastToday['wind'])
        # col6.metric('更新时间', forecastToday['updateTime'])

        # 轮播图  24小时内到7天内
        c1 = (
            Line()
            .add_xaxis(xaxis_data=df_forecastHours.index.to_list())
            .add_yaxis(series_name='温度', y_axis=df_forecastHours['Temperature'].values.tolist())
            .add_yaxis(series_name='体感温度', y_axis=df_forecastHours['Body Temperature'].values.tolist())
            .set_global_opts(
                title_opts=opts.TitleOpts(title="24小时内天气"),
        ...
        c2 = (
            ...
            .add_yaxis(series_name="最高温", y_axis=df_forecastDays.Temperature.apply(
                lambda x: int(x.replace('°C', '').split('~')[1])).values.tolist())
            .add_yaxis(series_name="最低温", y_axis=df_forecastDays.Temperature.apply(
                lambda x: int(x.replace('°C', '').split('~')[0])).values.tolist())
            .set_global_opts(
                title_opts=opts.TitleOpts(title="七天内天气"),
                xaxis_opts=opts.AxisOpts(type_="category"),
                yaxis_opts=opts.AxisOpts(type_="value", axislabel_opts=opts.LabelOpts(formatter="{value} °C")),
                tooltip_opts=opts.TooltipOpts(trigger="axis", axis_pointer_type="cross")
            )
        ...
        with st.expander("未来24小时天气预测"):
            st.table(df_forecastHours.style.format(
                {'Temperature': '{}°C', 'Body Temperature': '{}°C', 'Humidity Weather': '{}%'}))
        with st.expander("未来七天天气预测"):
            st.table(df_forecastDays)
        with st.expander("常用链接"):
            st.table(df_forecastDays)
        with st.expander("待办事项提醒"):
            st.table(df_forecastDays)
        with st.expander("昨夜今晨工厂动态"):
            st.write('跳转链接一', 'www.baidu.com')
            st.write('跳转链接二', 'www.thermos.com.cn')
...
def get_city_mapping():
    ...
    for i in data.values():
        for each in i:
            ...
            if each['name'] != "苏州市" and flag:
                suzhou += 1
            else:
                flag = False
    suzhou -= 1

    return city_mapping, suzhou
